...
class GaussianMixture:
    ...
    def plot(self, data=None):
        ...
        if data:
            ...
            ax[3].scatter(x, y, s=30, c=colors, alpha=0.5)
            ax[0].scatter(x[yis1], y[yis1], s=5, c=colors[yis1], alpha=0.3)
            ax[1].scatter(x[yis0], y[yis0], s=5, c=colors[yis0], alpha=0.3)
            ax[2].scatter(x, y, s=5, c=colors, alpha=0.3)
        pl.show()
...
def read_data(filename):
    with open(filename) as f:
        g = (
            ",".join([i[1], i[2], i[4], i[5], i[6], i[7], i[9], i[11]]).encode(
                encoding="UTF-8"
            )
...
def load_data():
    return load_data_("data/titanic_train.csv")


def load_test_data():
    return load_data_("data/titanic_test.csv")
...
def plot_svm(
    ...
    xx, yy = np.meshgrid(
        np.linspace(minbound[0], maxbound[0], resolution[0]),
...
def illustrate_preprocessing():
    x = np.random.multivariate_normal(
        np.array([5.0, 5.0]), np.array([[5.0, 3.0], [3.0, 4.0]]), size=1000
    )
...
def margins_and_hyperplane():
    ...
    X = (
        np.vstack(
            (
                np.ones((n, 2)) * np.array([0.5, 1]),
                np.ones((n, 2)) * np.array([-0.5, -1]),
            )
        )
        + np.random.randn(2 * n, 2) * 0.3
    )
    ...
    ax.scatter(
        clf.support_vectors_[:, 0], clf.support_vectors_[:, 1], s=80, facecolors="none"
    )
    ax.scatter([minp[0], maxp[0]], [minp[1], maxp[1]], s=80, facecolors="none")
